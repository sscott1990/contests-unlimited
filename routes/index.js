...
// ✅ Load uploads.json directly from S3
function loadUploadsFromS3(callback) {
  s3.getObject({
    Bucket: BUCKET_NAME,
    Key: 'uploads.json'
  }, (err, data) => {
    if (err) {
      console.error('Error loading uploads.json from S3:', err);
      return callback([]);
    }
    try {
      const uploads = JSON.parse(data.Body.toString('utf-8'));
      callback(uploads);
    } catch (e) {
      console.error('Invalid JSON in uploads.json:', e);
      callback([]);
    }
  });
}
...
router.get('/', (req, res) => {
  loadUploadsFromS3((uploads) => {
    ...
    // Updated prizeList: show entries based on prize amount divided by 2.5
    const prizeList = Object.entries(prizes).map(([contest, total]) =>
      `<li><strong>${contest}</strong>: $${total.toFixed(2)} — Entries: ${Math.floor(total / 2.5)}</li>`
    ).join('');
